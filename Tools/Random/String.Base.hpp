...
namespace Tools::Random {
    ...
    inline char RandomSChar(u32 From, u32 To) {
        CheckRange(From, To);
        return RandomChar<char>(From, To);
    }

    // Generate random wchar from codepoint range
    inline wchar RandomWChar(u32 From, u32 To) {
        CheckRange(From, To);
...
namespace Tools::Random {
    ...
    inline str RandomSStr(u32 From, u32 To, const idx Count) {
        CheckRange(From, To);
        return RandomStr<char>(From, To, Count);
    }

    // Generate Wide STRing std::wstring from given range
    inline wstr RandomWStr(u32 From, u32 To, const idx Count) {
        CheckRange(From, To);
...
namespace Tools::Random {
    ...
    str RandomSStr(const OneOf<vec<char>, str> auto& CharSet, const idx Count) {
        return RandomStr<char>(CharSet, Count);
    }

    // Generate Wide STRing std::wstring from charset (wstr, vec<wchar>)
    wstr RandomWStr(const OneOf<vec<wchar>, wstr> auto& CharSet, const idx Count) {
