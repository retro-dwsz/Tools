 ...
 * File "Tools/Experimental/VNum_Draft_1/VNum.hpp"
 */
...
class VNum {
    ...
    // Core unsigned magnitude operations (ignore sign)
    static vec<u8> AddMagnitudes(const vec<u8>& a, const vec<u8>& b);
    static vec<u8> SubMagnitudes(const vec<u8>& a, const vec<u8>& b); // assumes a >= b
    static int CompareMagnitudes(const vec<u8>& a, const vec<u8>& b); // -1, 0, +1

    // Decimal alignment helper
    static void AlignDecimals(
    ...
    pair<vec<u8>, vec<u8>> GetData() const {
        return {this->m_DataInt, this->m_DataDec};
    }
    ...
    private:
    // Multiply single digit with entire magnitude vector
    static vec<u8> MulByDigit(const vec<u8>& mag, u8 digit);
    // Add two magnitudes with left-shift (append zeros)
    static vec<u8> AddShifted(const vec<u8>& acc, const vec<u8>& partial, idx shift);
    public:
    [[nodiscard]] VNum operator*(const VNum& Other) const;

    private:
    // Compare and subtract: if a >= b, returns a-b and sets quotient digit
    // Returns true if subtraction was performed
    static bool TrySubtract(vec<u8>& remainder, const vec<u8>& divisor);
    public:
    [[nodiscard]] VNum operator/(const VNum& Other) const;
    ...
    /* To make re-assignment possible ?? */
    ...
    // Comparasion
    bool operator==(VNum& Other) const noexcept;        // Compare each digit and direction
    auto operator<=>(const VNum& Other) const noexcept; // C++20 spaceship!
    ...
    operator span<str>() const noexcept;    // span auto struct
