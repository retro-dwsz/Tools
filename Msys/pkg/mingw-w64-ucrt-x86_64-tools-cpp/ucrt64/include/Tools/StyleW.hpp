...
namespace Tools::StylingW {
    ...
    str ColorFG(const str& Text, uint32_t color_tx = 0xFF8A46, int alpha = 100) {
        uint8_t r, g, b;
        if (alpha < 100) {
            uint32_t blended_color = Tools::CommonW::BlendAlpha(color_tx, alpha);
            Tools::CommonW::BlendAlpha(r, g, b, blended_color);
        } else {
            Tools::CommonW::BlendAlpha(r, g, b, color_tx);
        }
    ...
    str ColorBG(const str& Text, uint32_t color_bg = 0x092655, int alpha = 100) {
        uint8_t r, g, b;
        if (alpha < 100) {
            uint32_t blended_color = Tools::CommonW::BlendAlpha(color_bg, alpha);
            Tools::CommonW::BlendAlpha(r, g, b, blended_color);
        } else {
            Tools::CommonW::BlendAlpha(r, g, b, color_bg);
        }
    ...
    /* inline */
    str ColorFG(const str& text, Tools::CommonW::Color rgb) {
        return std::format(
            "\033[38;2;{};{};{}m{}\033[0m",
    ...
    /* inline */
    str ColorBG(const str& text, Tools::CommonW::Color rgb) {
        return std::format(
            "\033[48;2;{};{};{}m{}\033[0m",
            std::to_string(rgb.r), std::to_string(rgb.g), std::to_string(rgb.b), text
        );
...
namespace Tools::StylingW {
    ...
    // Foreground color with opacity using struct Color
    /* DEP */ /* inline */
    wstr ColorFG(const wstr& Text, Tools::CommonW::Color rgb) {
        return wstr(L"\033[48;2;")
    ...
    // Background color with opacity using struct Color
    /* DEP */ /* inline */
    wstr ColorBG(const wstr& Text, Tools::CommonW::Color rgb) {
        return wstr(L"\033[48;2;")
