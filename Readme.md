# Retro Dave's Tools "RDTools.hpp" rdt:: (or rdt::)
...
- ### Folders
    ...
    |`/Tools/TrueRandom/*`  | OS-level random number implementations (Win32 Only)   |
...
- ### Folder `Tools/TrueRandom/*`
    ...
    | ~                                     | ~                                                                             |
    | `/TrueRandom/Single.hpp`              | Singly number generator                                                       |
    | `/TrueRandom/Single.Impl.hpp`         | Singly number generator with specialized types                                |
    | ~                                     | ~                                                                             |
    | `/TrueRandom/Vector.hpp`              | Vector number generator                                                       |
    | `/TrueRandom/Vector.Impl.hpp`         | Vector number generator with specialized types                                |
    | `/TrueRandom/Bundle.hpp`              | Bundled number generator                                                      |
    | `/TrueRandom/Bundle.Impl.hpp`         | Bundled number generator with specialized types                               |
    | `/TrueRandom/SBundle.hpp`             | Scattered bundled number generator                                            |
    | `/TrueRandom/SBundle.Impl.hpp`        | Scattered bundled number generator with specialized types                     |
    | ~                                     | ~                                                                             |
    | `/TrueRandom/Vector.Thread.hpp`       | Vector number generator with multithreading                                   |
    | `/TrueRandom/Vector.Thread.Impl.hpp`  | Vector number generator with multithreading with specialized types            |
    | `/TrueRandom/Bundle.Thread.hpp`       | Bundled number generator with multithreading                                  |
    | `/TrueRandom/Bundle.Thread.Impl.hpp`  | Bundled number generator with multithreading with specialized types           |
    | `/TrueRandom/SBundle.Thread.hpp`      | Scattered bundled number generator with multithreading                        |
    | `/TrueRandom/SBundle.Thread.Impl.hpp` | Scattered bundled number generator with multithreading with specialized types |
    | ~                                     | ~                                                                             |
    | `/TrueRandom/Choice.hpp`              | Random choice/s from iterables                                                |
    | `/TrueRandom/String.hpp`              | Random char/string generators                                                 |
    | `/TrueRandom/String.Impl.hpp`         | Random char/string generators with specialized types                          |
...
- ### Folder `Tools/Lib` (Test only!)
    ...
    |            File           |              What is this?              |
    |---------------------------|-----------------------------------------|
    | `RDTools.Processed.win.hpp` | A preprocessed `RDTools.hpp` file, you may can use this as super poratable header-file, but I don't think you should|
    | `RDTools.Processed.win.o`   | Compiled object of `ToolsProcessed.hpp` |
    | `libRDTools.a`              | Packed with `ar rcs` of `RDTools.o` file  |
...
# `III`. Lib `RDTools.Edges`

- ### Description
    Should've named "limits", but I pick "Edges" just because people won't confuse `<limits>` and `RDTools.Limits`, so `RDTools.Edges` were chosen.
- ### API Code:
    ```c++
    #pragma once
...
- IVec.hpp
    ...
    Main entry point for the `rdt::ivec<T>` container library.

- IVec/Base.Master.hpp
    * Master aggregation header for the `rdt::ivec<T>` container.
...
- IVec/Base.Class.hpp
    ...
    ### Note
    I've reworked `RDTools.IVec` because old version is horrobly terrible even for some simple tasks. Yes, I know what are ye thinking. So in this new version, I reworked everything from base class typing into segmented typing. Why? Because I hate my self and C++ it self.

    > ### Brief:
    > > - Primary class declaration for `rdt::ivec<T>` (declarations only).
    >
    > ### Details:
...
- IVec/Init.Ctor.hpp
    ...
    > ### Brief:
    > > Constructor implementations for `rdt::ivec<T>`.
    >
    > ### Details:
...
- IVec/Init.Dtor.hpp
    > ### Brief
    > > Destructor implementation for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Core.Data.hpp
    > ### Brief
    > > Core data accessors for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Core.State.hpp
    > ### Brief
    > > Container state management for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Core.Resize.hpp
    > ### Brief
    > > Capacity management for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Core.Append.hpp
    > ### Brief
    > > Single-element append operations for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Core.Append.Idx.hpp
    > ### Brief
    > > Positional insertion operations for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Core.Emplace.hpp
    > ### Brief
    > > In-place construction at end for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Core.Emplace.Idx.hpp
    > ### Brief
    > > Positional in-place construction for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Core.Extend.hpp
    > ### Brief
    > > Bulk element insertion for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Access.Getset.hpp
    > ### Brief
    > > Element access operators and copy assignment for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Access.Pop.hpp
    > ### Brief
    > > Element removal operations for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Access.Elements.hpp
    > ### Brief
    > > Positional element accessors for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Access.Iter.hpp
    > ### Brief
    > > Pointer-based iterator accessors for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Access.Ref.hpp
    > ### Brief
    > > Bounds-checked reference accessors for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Opr.Dunder.hpp
    > ### Brief
    > > Python-style operator overloads for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Items.Orders.hpp
    > ### Brief
    > > Sorting, shuffling, and reversing operations for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Items.Query.hpp
    > ### Brief
    > > Deduplication and bulk reassignment operations for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Items.Find.hpp
    > ### Brief
    > > Linear search utilities for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Items.Slice.hpp
    > ### Brief
    > Sub-range extraction operations for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Items.Erase.Idx.hpp
    > ### Brief
    > > Index-based element removal for `rdt::ivec<T>`. Modern index-based erase are using `Erase` (with '`E`')
    >
    > ### Details
...
- IVec/Items.Erase.Ptr.hpp
    > ### Brief
    > Pointer-based element removal for `rdt::ivec<T>`. Legacy pointer-based erase are using `erase` (with '`e`')
    >
    > ### Details
...
- IVec/Functional.hpp
    > ### Brief
    > > Python-style functional transformations for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Converter.hpp
    > ### Brief
    > > String formatting and generic container conversion for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Converter.Opr.hpp
    > ### Brief
    > > Explicit type conversion operators for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Legacy.hpp
    > ### Brief
    > > STL-compatible and legacy API aliases for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Format.std.hpp
    > ### Brief
    > > Optional custom `std::formatter` for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Format.fmt.hpp
    > ### Brief
    > > Optional custom `fmt::formatter` for `rdt::ivec<T>`.
    >
    > ### Details
...
- IVec/Base.RangesQualification.hpp
    > ### Brief
    > > Compile-time C++20/26 ranges concept verification for `rdt::ivec<T>`.
    >
    > ### Details
...
- ### Example
    ```c++
    int result = rdt::Linking::CallFunction<int>(
        "Math.dll",
        "Add",
        5,
        3
...
# `X`. Lib `RDTools.StyleW`

- ### Description:
    - Same as `RDTools.Style`, but with `std::wstring` (`wstr`) support, slightly different choise of diction, but still good, and maybe kinda simpler. Just add 'W' at the end of function name.
...
- ### Examples:
    - `rdt::Styling::PrintMid("[ My Cool C++ App! ]", '~')`
    ...
    - `rdt::Styling::PrintMid(" My magcial C++ App! ", '/')`
...
# `XII`. Lib `RDTools.Random`
 * ### Code suffixes you'll find on Random functions
    | **Suffix**| **Meaning**      | **Types** | **Meaning**                    |
    |-----------|------------------|-----------|--------------------------------|
    | `I`       | Integer (`i32`)  | `V`       | Vector `T`                     |
    | `L`       | Long (`i64`)     | `B`       | Bundled `T`                    |
    | `F`       | Float (`f32`)    | `SB`      | Scattered Bundled `T`          |
    | `D`       | Double (`f64`)   | `TV`      | Vector Threaded `T`            |
    | `V`       | Vector           | `TSB`     | Threaded Scattered Bundled `T` |
    | `B`       | Bundled          |           |                                |
    | `SB`      | Scattered Bundle |           |                                |
...
- ### API Codes Synopsis:
    ...
    > ### Library Overview
    > RDTools.Random is a high-performance, C++26-compliant randomization wrapper
    ...
    ### Master file
    `RDTools.Random`, random number wrapper for singly/multiple items per function
    - Total files: 14
    ...
    This header implements the core "Single" functionality of RDTools.Random. It offers
    ...
    */
    ...
    This header implements the core "Vector" functionality of RDTools.Random. It generates
    ...
    */
    ...
    namespace rdt::Random {
         ...
         *
         * @tparam Int The integer type (must satisfy rdt::Types::Integer). Default: i32.
         ...
         *
         * @tparam Real The floating-point type (must satisfy rdt::Types::Float). Default: f32.
         * @param Gen Reference to custom Mersenne Twister engine (used for seeding workers).
         * @param ValueCount Total number of elements to generate. Default: 10.
         * @param ValueMin Lower bound of the range (inclusive). Default: -10.
         * @param ValueMax Upper bound of the range (inclusive). Default: 10.
...
# `XIII`. Lib `RDTools.TrueRandom`

### What is TRNG?
True Random Number Generator (TRNG) produces randomness from physical/OS entropy sources rather than mathematical algorithms. Output is fundamentally non-deterministic and non-reproducible.
This module uses Windows BCryptGenRandom(SYSTEM_PREFERRED_RNG), which combines hardware RNG, OS entropy pool, and FIPS 140-2 AES-CBC-MAC conditioning into a single cryptographically secure byte stream.
...
| Do                                     | Don't                                                                 |
|----------------------------------------|-----------------------------------------------------------------------|
| Security tokens, API keys, session IDs | Game physics, simulations, ML training use `rdt::Random`              |
| Password/salt generation               | Procedural generation needing reproducibility, use `rdt::Random`      |
| Cryptographic key material             | High-throughput bulk generation (>100K values/sec), use `rdt::Random` |
...
using namespace rdt::TrueRandom;
...
All functions satisfy cryptographic security requirements but are significantly slower than PRNG equivalents. Profile before using in hot paths.
...
# `XIV`. Lib `RDTools.Randomizer`

Library used internally for `RDTools.Random` and `RDTools.TrueRandom`, this just contain aliases for some objects.
...
- ### Example:
    ```c++
    f64 a = 3.14159;
    f64 b = rdt::Round::Round(a, 2);
    fmt::println("a = {} -> {}", a, b);  // a = 3.14159 -> 3.14
    ```

---

# `XVI`. Lib `RDTools.Vector`
...
- ### Lifetime Warning
    - PStr does NOT copy the underlying string data. The caller MUST ensure that the pointed-to string outlives the PStr instance. Dangling pointers will cause UB.
...
- ### API Codes
    ...
    namespace rdt::Time {
        ...
        // Sleep for given duration in `rdt::Time::Units::*` unit
        template <Duration T>
...
- ### Example of usage
    ...
    2. Run function directly from `Executor`
        ...
        i32 main() {
            ...
            auto timeMs = rdt::Time::FunctionElapsed<rdt::Time::Units::ms>([] {
